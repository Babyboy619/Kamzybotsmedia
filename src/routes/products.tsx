...
import { listExternalLogs, type ExternalLogProduct } from "@/lib/api/logs";
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    const category = dbCategories.find((c) => c.slug === activeCat);
    if (!category) {
    ...
    void (async () => {
      try {
        const externalLogs = await listExternalLogs();
        const relevant = externalLogs.filter(
          (item) =>
            item.title.toLowerCase().includes(category.name.toLowerCase()) ||
            item.slug.toLowerCase().includes(category.slug.toLowerCase()),
        );
        setProducts(relevant.length > 0 ? relevant : externalLogs);
      } catch {
        setProducts([]);
      } finally {
        setProductsLoading(false);
    ...
    <>
      ...
      <section className="w-full bg-background py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h3 className="text-lg font-semibold text-brand-foreground/80">
            ...
            {displayCategories.map((c) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={`group p-3 rounded-lg flex flex-col items-center justify-center space-y-2 ${meta.bg} ${active ? "ring-2 ring-offset-2 ring-brand-500" : ""}`}
                >
