...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from("orders")
        .select("*, order_items(title, quantity, unit_price)")
        .eq("user_id", user.id)
                ...
                <Card className="border-brand-orange/20 bg-brand-orange/5">
                  ...
                  <CardContent>
                    ...
                    <Button
                      asChild
                      size="sm"
                      className="mt-3 bg-brand-orange hover:bg-brand-orange-hover text-white text-xs"
                    >
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <Badge
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[order.status] ?? "bg-gray-100 text-gray-700"}`}
                >
                  {order.status}
                </Badge>
              </div>
          ...
          <div className="py-2">
            {credsLoading ? (
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
                ...
                <p>
                  Credentials for this order haven't been assigned yet. Please check back shortly or
                  contact support.
                </p>
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  {creds.length} credential{creds.length > 1 ? "s" : ""} delivered.
                </p>
                {creds.map((c) => (
                  <div key={c.id} className="rounded-xl border border-border overflow-hidden">
                    {c.label && (
                      <div className="bg-muted/50 px-4 py-2 text-xs font-medium text-brand-navy border-b border-border">
                        {c.label}
                      </div>
                    )}
                    <div className="relative p-4 bg-muted/20">
                        ...
                        <div className="space-y-2">
                          {parseCredential(c.content).map(({ label, value }) => (
                            <div key={label} className="flex items-start gap-2 text-sm">
                              <span className="text-xs font-semibold text-brand-navy w-28 shrink-0 pt-0.5">
                                {label}
                              </span>
                      ...
                      >
                        {copied === c.id ? (
                          <>
                            <CheckCheck className="w-3.5 h-3.5 mr-1 text-green-500" />
                            Copied!
                          </>
                        ) : (
                          <>
                    ...
                    {c.delivered_at && (
                      <div className="bg-muted/30 px-4 py-1.5 text-xs text-muted-foreground border-t border-border">
                        Delivered{" "}
                        {new Date(c.delivered_at).toLocaleString("en-NG", {
                          dateStyle: "medium",
                          timeStyle: "short",
                        })}
                      </div>
...
function ProfileTab({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle className="text-brand-navy text-base">Profile Information</CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-brand-navy text-base">Change Password</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-muted-foreground text-sm mb-6">
        Reach our team or join the community for updates and help.
      </p>
        ...
        {links.map(({ icon: Icon, label, href, color, bg, desc }) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-brand-navy group-hover:text-brand-orange transition-colors text-sm">
                {label}
              </div>
