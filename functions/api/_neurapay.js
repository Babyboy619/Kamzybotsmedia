...
//
// NeuraPay API surface (probed against https://neurapay.com.ng/api/v1):
//   GET  /api/v1/transactions/init    -> 401 Unauthenticated without a key
//   GET  /api/v1/transactions/verify  -> 401 Unauthenticated without a key
//   POST /api/v1/transactions/*       -> 405 "Supported methods: GET, HEAD"
// The previous implementation used POST, which NeuraPay rejects with 405.
// Method and paths are env-overridable so they can be corrected from the
...
export function neuraPayConfig(env) {
  ...
  return {
    ...
    initPath: (readEnv(env, "NEURAPAY_INIT_PATH") || "transactions/init").replace(/^\/+/, ""),
    verifyPath: (readEnv(env, "NEURAPAY_VERIFY_PATH") || "transactions/verify").replace(/^\/+/, ""),
    // NeuraPay currently answers GET/HEAD only on these routes.
 ...
 */
export async function neuraPayRequest(cfg, path, params) {
  ...
  const init = { method: cfg.method, headers, signal: AbortSignal.timeout(NEURAPAY_TIMEOUT_MS) };
