// Cloudflare Pages Function — POST /api/categories/upsert
// Admin-only: create or update a product category bypassing RLS.
...
export async function onRequestPost({ request, env }) {
  ...
  if (id) {
    const res = await sbFetch(supabaseUrl, serviceKey, `/rest/v1/product_categories?id=eq.${id}`, {
      method: "PATCH",
    ...
    return json({ category: Array.isArray(data) ? data[0] : data });
  }

  const res = await sbFetch(supabaseUrl, serviceKey, "/rest/v1/product_categories", {
    method: "POST",
    headers: { "Content-Type": "application/json", Prefer: "return=representation" },
    body: JSON.stringify(payload),
  });
